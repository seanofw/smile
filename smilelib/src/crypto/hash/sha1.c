...
)
{
	...
	if ((j + BufferSize) > 63)
	{
		...
		{
			TransformFunction(Context->State, (Byte*)Buffer + i);
		}
		j = 0;
	}
	else
	...
	memcpy(&Context->Buffer[j], &((Byte*)Buffer)[i], BufferSize - i);
}
...
)
{
	...
	Sha1_Update(Context, (Byte*)"\x80", 1);
	while ((Context->Count[0] & 504) != 448)
	{
		Sha1_Update(Context, (Byte*)"\0", 1);
	}

	Sha1_Update(Context, finalcount, 8);  // Should cause a Sha1TransformFunction()
...
void Sha1(Byte *result, const Byte *buffer, Int size)
{
	Sha1Context context;
	Sha1_Init(&context);
