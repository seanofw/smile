...
static void PointerDictInt_Resize(struct PointerDictInt *self, Int newLen)
{
	struct PointerDictNode *newHeap, *oldHeap;
	...
	if (newLen >= IntMax || newLen >= IntMax / sizeof(struct PointerDictNode)) Smile_Abort_OutOfMemory();
	newBuckets = GC_MALLOC_RAW_ARRAY(Int, newLen);
	if (newBuckets == NULL) Smile_Abort_OutOfMemory();
	newHeap = GC_MALLOC_STRUCT_ARRAY(struct PointerDictNode, newLen);
	...
	for (oldBucketIndex = 0; oldBucketIndex < oldLen; oldBucketIndex++) {
		...
		for (oldNodeIndex = oldBuckets[oldBucketIndex]; oldNodeIndex >= 0; oldNodeIndex = oldHeap[oldNodeIndex].next) {
			newBucketIndex = (PtrInt)oldHeap[oldNodeIndex].key & newMask;
...
Int PointerDictInt_Append(struct PointerDictInt *self, const void *key, const void *value)
{
	struct PointerDictNode *heap;
	Int nodeIndex, bucketIndex;

	if (key == NULL) return -1;

	if (self->firstFree < 0) {
		PointerDictInt_Resize(self, (self->mask + 1) * 2);
	}

	heap = self->heap;

	nodeIndex = self->firstFree;
	bucketIndex = (PtrInt)key & self->mask;
	self->firstFree = heap[nodeIndex].next;

	heap[nodeIndex].next = self->buckets[bucketIndex];
	heap[nodeIndex].key = (void *)key;
...
/// This will be the same length as returned by PointerDict_GetCount().</returns>
PointerDictKeyValuePair *PointerDict_GetAll(PointerDict stringDict)
{
	struct PointerDictInt *self;
	Int bucket, nodeIndex;
	Int *buckets;
	PointerDictKeyValuePair *pairs, *dest;
	struct PointerDictNode *heap, *node;

	self = (struct PointerDictInt *)stringDict;

	pairs = GC_MALLOC_STRUCT_ARRAY(PointerDictKeyValuePair, self->count);
...
/// This will be the same length as returned by PointerDict_GetCount().</returns>
void **PointerDict_GetKeys(PointerDict stringDict)
{
	struct PointerDictInt *self;
	Int bucket, nodeIndex;
	Int *buckets;
	...
	keys = GC_MALLOC_STRUCT_ARRAY(void *, self->count);
...
/// This will be the same length as returned by PointerDict_GetCount().</returns>
void **PointerDict_GetValues(PointerDict stringDict)
{
	struct PointerDictInt *self;
	Int bucket, nodeIndex;
	Int *buckets;
...
void PointerDict_ClearWithSize(PointerDict stringDict, Int newSize)
{
	struct PointerDictInt *self;
	struct PointerDictNode *heap;
	Int *buckets;
	...
	self->heap = heap = GC_MALLOC_STRUCT_ARRAY(struct PointerDictNode, newSize);
...
Bool PointerDict_Remove(PointerDict stringDict, const void *key)
{
	struct PointerDictInt *self;
	struct PointerDictNode *heap;
	Int *buckets;
	...
	mask = self->mask;
	nodeIndex = buckets[(PtrInt)key & mask];
	prevIndex = -1;
	...
	while (nodeIndex >= 0) {
		if (heap[nodeIndex].key == key) {
			if (prevIndex >= 0)
				...
				buckets[(PtrInt)key & mask] = heap[nodeIndex].next;
			...
			if (self->count <= ((mask + 1) >> 2) && (mask + 1) > 16) {
				PointerDictInt_Resize(self, (mask + 1) >> 1);
			}
			return True;
		}
...
SMILE_API_FUNC DictStats PointerDict_ComputeStats(PointerDict stringDict)
{
	struct PointerDictInt *self;
	struct PointerDictNode *heap;
	...
	for (bucketIndex = 0; bucketIndex <= self->mask; bucketIndex++) {
		...
		for (nodeIndex = buckets[bucketIndex]; nodeIndex >= 0; nodeIndex = heap[nodeIndex].next) {
			numInThisBucket++;
			SimpleStats_Add(stats->keyStats, (Float64)SizeofPtrInt);
		}
